...
const CandlestickChart = ({ children, data, coinId, height = 360, initialPeriod = "daily", liveOhlcv = null, mode = "historical", liveInterval, setLiveInterval }: CandlestickChartProps) => {
  ...
  //additional ref to able to update the chart in real time
  ...
  const fetchOHLCData = async (selectedPeriod: Period) => {
    try {
      ...
      const newData = await fetcher<OHLCData[]>(`/coins/${coinId}/ohlc`, {
              vs_currency: "usd",
              days: days,
              //those two below does not work on the free demo plan
              //  interval: interval,
              // precision: "full",
            })
  ...
  useEffect(() => {
    ...
    //clean everything up by the end
    return () => {
      observer.disconnect();
      chart.remove();
